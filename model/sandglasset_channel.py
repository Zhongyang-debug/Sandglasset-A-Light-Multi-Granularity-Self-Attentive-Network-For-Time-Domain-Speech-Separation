...
class Sandglasset_Block(nn.Module):
    ...
    def __init__(self, in_channels, channels, hidden_channels=128, num_layers=1, bidirectional=True, num_heads=8,
        ...
        self.Globally_Attentive = Globally_Attentive(in_channels=channels,
                                                     num_heads=num_heads)

        self.ConvTrans1D = nn.ConvTranspose1d(in_channels=channels,
    ...
    def forward(self, x):
        ...
        B, N, K, S = x.shape  # torch.Size([1, 64, 256, 64])
        ...
        x = self.Conv1D(x)
        _, N1, K1 = x.shape
        x = x.reshape(B, S, N1, K1).permute(0, 2, 3, 1)

        x = self.Globally_Attentive(x)

        # 上采样
        x = x.permute(0, 3, 1, 2).reshape(B*S, N1, K1)
    ...
    def __init__(self, in_channels, out_channels, length, hidden_channels=128, num_layers=1, bidirectional=True,
        ...
        for i in range(self.cycle_amount):
            if i < self.cycle_amount//2:
                channels.append(out_channels*(4**i))
                kernel_size.append(4**i)
                stride.append(4**i)
            else:
                channels.append(out_channels*(4**(self.cycle_amount-i-1)))
